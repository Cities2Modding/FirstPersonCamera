...
namespace FirstPersonCamera.MonoBehaviours
{
    ...
    public class FirstPersonCameraController : MonoBehaviour
    {
        ...
        public void Initialise( FirstPersonCameraSystem firstPersonCameraSystem )
        {
            _firstPersonCameraSystem = firstPersonCameraSystem;

            var world = _firstPersonCameraSystem.World;
            terrainSystem = world.GetExistingSystemManaged<TerrainSystem>( );
            cameraUpdateSystem = world.GetExistingSystemManaged<CameraUpdateSystem>( );
        ...
        {
            var childObject = new GameObject( "FirstPersonCamera_SubObject" );
        ...
        private void Toggle( )
        {
            ...
            if ( IsActive )
            {
                ...
                renderingSystem.levelOfDetail = storedLODScale * 0.75f;
            }
            else
            {
        ...
        private float GetGroundY( )
        {
            try
            {
                ...
                return TerrainUtils.SampleHeight( ref heightData, new float3( transform.position ) ) + 1.7f; // Offset it a little
            }
